...
//	}
//
//	c := collector.NewAPICollector(fake.NewFakeLogger(), mattermostEndpointURL)
//
...
func TestDriver_collectMetricsHealth(t *testing.T) {
	...
	tests := []struct {
		name            string
		want            domain.MetricsDataHealth
		apiMockResponse httpmock.Responder
	}{
		{
			name:            "all is good",
			want:            domain.MetricsDataHealth{SystemHealth: true, SystemHealthFilestore: true, SystemHealthDatabase: true},
		...
		},
		{
			...
			want:            domain.MetricsDataHealth{SystemHealth: false, SystemHealthFilestore: false, SystemHealthDatabase: true},
		...
		{
			name:            "API send a 500 error",
			want:            domain.MetricsDataHealth{SystemHealth: false, SystemHealthFilestore: false, SystemHealthDatabase: false},
			apiMockResponse: httpmock.NewStringResponder(500, `{"error":"unexpected error"`),
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			metrics := c.collectHealth(context.Background())
...
//	}
//
//	c := NewAPICollector(fake.NewFakeLogger(), mattermostEndpointURL)
//
//	for _, tt := range tests {
